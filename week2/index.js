...
var l = "8";
// 請文字解釋為什麼
console.log(k * l > 21); //  L 會先轉成 NUMBER 然後跟 3 乘 , 所以會變成 3*8 > 21
...
if (BobIsVip == true && giftPriceRule >= 1599) {
  console.Log("客戶您好，您有符合贈品資格");
} else {
  console.Log("客戶您好，您沒有符合贈品資格");
}
// Bob 拿不到 因為沒有 VIP 資格 故贈還是280個
...
if (coachIncome > 300000) {
  coachBonus = 240000 * 0.2 + 6000;
} else if (coachIncome > 100000 && coachIncome < 300000) {
  coachBonus = 240000 * 0.15 + 6000;
} else {
  coachBonus = 240000 * 0.1 + 6000;
}
...
const gymCoach = [
  {
    ...
    background:
      "擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員",
    register: true,
    singleClassHours: 60,
    singleClassPrice: 2000,
    groupClassHours: 90,
    groupClassPrice: 1500,
  },
  {
    name: "李教練",
    specialSkill: ["瑜伽", "體態雕塑"],
    background:
      "是一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員",
    register: false,
    singleClassHours: 50,
    singleClassPrice: 1800,
    groupClassHours: 75,
    groupClassPrice: 1200,
  },
]; // 練習：使用物件變數定義兩位教練的資訊
...
if (performanceData.coaches.name == "Alice") {
  performanceData.coaches.performance += 8000;
}

// 練習：第二位教練（可將下方程式碼註解移除，完成答題）

if (performanceData.coaches.name == "Bob") {
  performanceData.coaches.performance += 12000;
}
...
// 練習：計算小明今日消耗的卡路里
calorieBurn += activities.bicycle.caloriesBurn * 10;
calorieBurn += activities.runner.caloriesBurn * 30;
calorieBurn += activities.yoga.caloriesBurn * 40;
calorieBurn += activities.bicycle.caloriesBurn * 10;
